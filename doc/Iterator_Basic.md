### 1. Iterator
Iterator는 자바스크립트 뿐만 아니라 다른 언어들에도 있는 개념으로 일종의 루프를 생각하면 이해하기 쉽다.   
...
```
values 메서드를 사용하여 Iterator를 생성했다.  
next를 사용할 때 마다 그 다음 값으로 넘어가며 모든 값을 순회하고 나면 done이 true로 나오며 끝난다.   
한번 끝난 Iterator는 다시 돌아가지 않으며 value가 undefined로 리턴한다.   
이 때문에 for of 루프가 가능한 것이며 일반 오브젝트가 루프를 순회할 수 없는 이유이기도 하다.   
...
Iterator란 프로토콜의 하나이므로 일반 오브젝트에 이터레이터 프로토콜을 적용하면 이터레이블 오브젝트로 만들 수 있다.   
이때 사용되는 것이 `Symbol.iterator`이다.
Iterable 오브젝트로 만들려면 `Symbol.iterator`와 `value, done`이 포함된 오브젝트를 반환하는 `next 메서드`를 가진 객체를 반환하면 된다.   
...
```
arr에는 일반적인 배열을 할당하였고 iterableObj에는 Symbol.iterator를 호출하여 이터레이터 오브젝트를 생성하였다.   
arr은 prototype이 `Array`이지만, iterableObj는 `Array Iterator`라는 이터레이터 오브젝트이다.   
